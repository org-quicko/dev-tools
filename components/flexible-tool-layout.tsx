...
export function FlexibleToolLayout({ columns, topControls, className }: FlexibleToolLayoutProps) {
  ...
  return (
    <div className={cn("flex flex-col w-full h-full max-h-[calc(100vh-6rem)]", className)}>
      {topControls && <div className="mb-3">{topControls}</div>}

      <div className={cn("flex-1 flex flex-col md:grid gap-3 min-h-0 max-h-full", gridCols)}>
          ...
          >
            <CardHeader className="p-2 sm:p-3 flex flex-row items-center space-y-0 gap-2 border-b shrink-0">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <CardTitle className="text-sm truncate">{column.title}</CardTitle>
              </div>
              {column.actions && (
            ...
            <CardContent className="p-0 flex-1 overflow-hidden">
              <ScrollArea className="h-full w-full p-2 sm:p-3">{column.content}</ScrollArea>
            </CardContent>
          </Card>
        ))}
      </div>
...
export function ColumnEmptyState({
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center text-center p-3 space-y-3">
