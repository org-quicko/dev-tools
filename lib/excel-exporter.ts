...
async function createComparisonSheet(
  ...
  try {
    ...
    reportData.forEach((row, index) => {
      try {
        const dataRow: (string | number)[] = [row.changeType, row.jsonPath, row.key]

        if (config.includeLineNumbers !== false) {
          dataRow.push(row.json1Line || "")
        }

        dataRow.push(row.json1Value)

        if (config.includeLineNumbers !== false) {
          dataRow.push(row.json2Line || "")
        }

        dataRow.push(row.json2Value)
